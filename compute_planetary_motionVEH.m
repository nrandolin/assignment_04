...
function [V_list,E_list,H_list] = compute_planetary_motionVEH(t_list,V0,orbit_params)
    ...
    E_list = zeros(num_pts,1);
    H_list = zeros(num_pts,1);
    ...
    for n = 1:num_pts
        ...
        E_list(n)=0.5*m_planet*(vx_out^2+vy_out^2)-(m_sun*m_planet*G/abs(x_out+y_out*sqrt(-1)));
        H_list(n)=m_planet*(x_out*vy_out-y_out*vx_out);
    end
